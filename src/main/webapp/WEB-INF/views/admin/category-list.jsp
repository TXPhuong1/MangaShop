...
<body>
					...
					<div class="aside-menu flex-column-fluid">
						<div class="hover-scroll-overlay-y px-2 my-5 my-lg-5" id="kt_aside_menu_wrapper" data-kt-scroll="true" data-kt-scroll-height="auto" data-kt-scroll-dependencies="{default: '#kt_aside_toolbar, #kt_aside_footer', lg: '#kt_header, #kt_aside_toolbar, #kt_aside_footer'}" data-kt-scroll-wrappers="#kt_aside_menu" data-kt-scroll-offset="5px">
							<div class="menu menu-column menu-title-gray-800 menu-state-title-primary menu-state-icon-primary menu-state-bullet-primary menu-arrow-gray-500" id="#kt_aside_menu" data-kt-menu="true">
								...
								<div class="menu-item">
									<a class="menu-link active" href="/admin/category">
										<span class="menu-icon">
											<span class="svg-icon svg-icon-2">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
													<path d="M21 10H13V11C13 11.6 12.6 12 12 12C11.4 12 11 11.6 11 11V10H3C2.4 10 2 10.4 2 11V13H22V11C22 10.4 21.6 10 21 10Z" fill="black" />
								...
								<div class="menu-item">
									<a class="menu-link" href="/admin/add-category">
										<span class="menu-icon">
											<span class="svg-icon svg-icon-2">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
													<path d="M21 10H13V11C13 11.6 12.6 12 12 12C11.4 12 11 11.6 11 11V10H3C2.4 10 2 10.4 2 11V13H22V11C22 10.4 21.6 10 21 10Z" fill="black" />
				...
				<div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
					<div id="kt_header" style="" class="header align-items-stretch">
						<div class="header-brand">
							<a href="/">
								<h1 style="color: white">Page Palette</h1>
							</a>
										...
										<div class="card-title">
											<div class="d-flex align-items-center position-relative my-1">
												...
												<form action="/admin/category" method="get">
													<input type="text" data-kt-user-table-filter="search" name="name" value="${name}" class="form-control form-control-solid w-250px ps-14" placeholder="Search category" />
												</form>
											</div>
										</div>
									</div>
											...
											<tbody class="text-gray-600 fw-bold">
												<c:forEach items="${categoryList}" var="c">
													<tr>
														<td>${c.id}</td>
														<td>${c.name}</td>
														<td class="${c.isDelete?'text-warning':'text-success'}">${c.isDelete?'Inactive':'Active'}</td>
														<td class="text-end">
														<td class="text-end">
															<a href="#" data-bs-toggle="modal" data-bs-target="#userModal" onclick="editCategory('${c.id},${c.name},${c.isDelete}');" class="btn btn-warning btn-active-light-primary btn-sm p-1">
																<i class="p-0 bi bi-pencil-square"></i>
															</a>
															<a href="/admin/category/delete/${c.id}" class="btn btn-danger btn-active-light-primary btn-sm p-1">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form:form action="/admin/category/edit" method="post" modelAttribute="categoryEdit">
						<div class="mb-3">
							<label for="exampleInputEmail1" class="form-label">ID <span class="form-text text-danger">*</span></label>
							<form:input id="id" path="id" readonly="true" type="text" class="form-control" required="required"/>
						</div>
						<div class="mb-3">
	...
	<script type="text/javascript">
		function editCategory(c) {
			...
			document.getElementById("id").value = category[0];
			document.getElementById("name").value = category[1];
			document.getElementById("isDelete").checked = (category[2]=='true')?true:false;
		}

		function alertCategory(title, message, status){
			swal(
					title,
					message,
					status
		...
		<script type="text/javascript">
			alertCategory('Error!', 'Delete fail!', 'error');
		</script>
		</c:if>
		<c:if test="${not empty success_category_delete}">
		<script type="text/javascript">
			alertCategory('Success!', 'Delete success!', 'success');
		</script>
		</c:if>
		<c:if test="${not empty error_category_update}">
		<script type="text/javascript">
			alertCategory('Error!', 'Update fail!', 'error');
		</script>
		</c:if>
		<c:if test="${not empty success_category_update}">
		<script type="text/javascript">
			alertCategory('Success!', 'Update success!', 'success');
		</script>
		</c:if>
	</script>	
